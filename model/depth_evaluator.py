...
if __name__ == '__main__':
    ...
    policy_model_path = args.model

    # Path to save the metrics to
    metric_directory = f"./metrics/{policy_model_path.split('/')[-1]}/{test_data_path.split('/')[-1].split('.')[0]}"
    if not os.path.exists(metric_directory):
        os.makedirs(metric_directory)
    metrics_path = metric_directory + "/metrics.json"
    # if the metrics_path doesn't exist, create the directory
    ...
    assert not (
                "reward" in eval_method and "mcqa" in eval_method), "You cannot evaluate both reward and mcqa at the same time!"
        ...
        for method in eval_method:
            if method == "rag":
                ...
                assert os.path.isdir(
                    "documents"), "You must have a directory named 'documents' with all the documents used for RAG, in the current directory."
