...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # =============================================================
        output = self.pretrained_model.generate(input_ids, attention_mask=attention_mask, max_length=1000, output_logits=True, return_dict_in_generate=True)
        output_dict["logits"] = output.logits
        output_dict["sequences"] = output.sequences
        ###############################################################
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        encoded_prompt = tokenizer.batch_encode_plus(batch["prompt"], return_tensors='pt', padding=True).to(self.device)
        encoded_chosen = tokenizer.batch_encode_plus(batch["chosen"], return_tensors='pt', padding=True).to(self.device)
        encoded_rejected = tokenizer.batch_encode_plus(batch["rejected"], return_tensors='pt', padding=True).to(self.device)
    ...
    def prediction_step_reward(
        ...
        output_dict["chosen_rewards"] = beta * (policy_chosen_logps - reference_chosen_logps)
        output_dict["rejected_rewards"] = beta * (policy_rejected_logps - reference_rejected_logps)
        ########################################################################
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        encoded_data = tokenizer.batch_encode_plus(questions, return_tensors='pt', padding=True)
        encoded_data = encoded_data.to(self.device)

        outputs = self.pretrained_model.generate(**encoded_data, max_length=500)
        answer = tokenizer.batch_decode(outputs, skip_special_tokens=True)

        answer = [answer[i][lengths[i]:] for i in range(len(answer))]

        tmp = []
        letter_list = ['A', 'B', 'C', 'D']
        for item in answer:
            temp_ans = re.search(r'[A-Z][ .,;:)]', item)
            if temp_ans is not None:
                # if we find a letter as answer, we append it
                tmp.append(temp_ans.group()[0])
            else:
                self.cnt_answer_not_found += 1
                # otherwise we append a random letter
                tmp.append(letter_list[torch.randint(0, 4, (1,)).item()])
        answer = tmp
        output_dict["preds"] = answer

        print(f"# answers not found: {self.cnt_answer_not_found}")
        ########################################################################
